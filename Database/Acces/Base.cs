...
namespace Database.Acces
{
	public class Base
	{
		...
		protected Classes.Base _classesBase;
		#endregion
		...
		public List<Classes.Base> Lire(string index)
		{
			CreerCommande("Lire");
			...
			List<Classes.Base> liste = AssigneChamp(sqlDataReader);
		...
		public Classes.Base LireId(int id)
		{
			...
			Classes.Base element = AssigneChamp(sqlDataReader)[0];
		...
		/// </summary>
		/// <returns>Une liste des enregistrements trouvés</returns>
		protected List<Classes.Base> AssigneChamp(SqlDataReader sqlDataReader)
		{
			Type type = _classesBase.GetType(); // récupère le type de l'objet

			List<Classes.Base> liste = new List<Classes.Base>(); // crée la liste à retourner
			...
			while (sqlDataReader.Read()) // parcourt toutes les données SQL reçues
			{
				...
				liste.Add(classe as Classes.Base); // ajoute l'objet à la liste de retour
			}
